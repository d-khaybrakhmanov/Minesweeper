...
class MineSwepper:
    ...
    def __init__(self):
        ...
        for i in range(MineSwepper.ROW + 2):
            ...
            for j in range(MineSwepper.COLUMNS + 2):
                btn = MyButton(MineSwepper.window, x=i, y=j, number=count)
                btn.config(command=lambda button=btn: self.click(button))
                btn.bind("<Button-2>", self.rigth_click)
                temp.append(btn)
                count += 1
    ...
    def click(self, clicked_button: MyButton):
        ...
        if clicked_button.is_mine:
            clicked_button.config(text='*', foreground='red',
            ...
            if cur_btn.count_bomb == 0:
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ...
                        if not next_btn.is_open and 1 <= next_btn.x <= \
                                MineSwepper.ROW and 1 <= next_btn.y <= \
                                MineSwepper.COLUMNS and next_btn not in queue:
    ...
    def create_settings_win(self):
        ...
        win_settings.wm_title('Настройки игры')
        ...
        column_entry.insert(0, MineSwepper.COLUMNS)
        ...
        save_btn = tk.Button(win_settings, text='Применить',
                  command=lambda :self.change_settings(row_entry,
                                                       column_entry,
                                                       mines_entry))
